  ...
  <n-form ref="formRef" :model="model" :rules="rules" :size="size" label-placement="top">
    <n-grid :cols="24" :x-gap="24">
      <n-form-item-gi :span="12" label="議題篩選" path="selectValue">
        <n-select filterable tag v-model:value="model.selectValue" placeholder="請選擇篩選條件" :options="generalOptions" />
      </n-form-item-gi>

      <n-form-item-gi :span="12" label="議題" path="selectValue">
        <n-select v-model:value="model.selectValue" placeholder="請選擇議題" :options="generalOptions" />
      </n-form-item-gi>

      <n-form-item-gi :span="12" label="開始日期" path="dateValue">
        <n-date-picker v-model:value="model.dateValue" type="date" placeholder="請選擇日期" />
      </n-form-item-gi>

      <n-form-item-gi :span="12" label="花費時間" path="inputNumberValue">
        <n-input-number v-model:value="model.inputNumberValue" placeholder="請輸入天數" />
        <span class="spendUnit">天</span>
        <n-input-number v-model:value="model.inputNumberValue" placeholder="請輸入時數" />
        <span class="spendUnit">小時</span>
        <n-input-number v-model:value="model.inputNumberValue" placeholder="請輸入分鐘數" />
      ...
      <n-form-item-gi :span="12" label="工作描述" path="textareaValue">
        <n-input v-model:value="model.textareaValue" placeholder="請簡單描述工作內容" type="textarea" :autosize="{
          minRows: 3,
          maxRows: 5,
        }" />
      </n-form-item-gi>
